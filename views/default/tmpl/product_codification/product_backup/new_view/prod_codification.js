var CODIFICATION = {
    ...
    pimnitemRow: function (olddata) {
        if(olddata.id != undefined && olddata.id != 0){
        ...
        if(CODIFICATION.cmpnt[i].id == olddata.ccnid){
        ...
        if(olddata.prod_quantity != undefined && olddata.prod_quantity != 0){
        ...
        htm += '</select>';
        htm += '</div>';
        htm += '<div class="col-md-3 forging-cls" style="float: left;text-align: center;display:none">';
        htm += '<select class="form-control i-r-cmpnt-forging" >';
        htm += '<option value="">select</option>'
        for(var i in CODIFICATION.forg){
        if(CODIFICATION.forg[i].id == olddata.forging){
       ...
       // htm += '<div class="form-row bar-cls" style="display: none">'
        htm += '<div class="col-md-1 bar-cls" style="float: left;text-align: center;display: none">';
        if(olddata.diameter != undefined && olddata.diameter != 0){
        htm += '<input type="text"  class="form-control i-r-diameter" placeholder="Diameter" value="'+olddata.diameter+'">';
        }
        else{
        htm += '<input type="text"  class="form-control i-r-diameter" placeholder="Diameter" >';
        }
        htm += '</div>';
        htm += '<div class="col-md-1 bar-cls" style="float: left;text-align: center;display: none">';
        if(olddata.length != undefined && olddata.length != 0){
        htm += '<input type="text"  class="form-control i-r-lngth" placeholder="Length" value="'+olddata.length+'" >';
        }
        else{
        htm += '<input type="text"  class="form-control i-r-lngth" placeholder="Length" >';  
        }
        htm += '</div>';
        htm += '<div class="col-md-1 bar-cls" style="float: left;text-align: center;display: none">';
        ...
        if(CODIFICATION.punit[i].id == olddata.punit){
        ...
        htm += '</div>';
       // htm += '</div>';
        htm += '<div class="col-md-3 casting-cls" style="float: left;text-align: center;display:none">'
        htm += '</div>'
        htm += '<div class="col-md-1" style="float: left;">';
    ...
    submitComponentFrm: function () {
        ...
        $( ".data-row-wrap" ).each(function( i ) {
            ...
            if(id>0){
                ...
                var cmpnttype = $(this).find('.i-r-cmpnt-type').val().trim(),forging = $(this).find('.i-r-cmpnt-forging').val().trim(),diameter = $(this).find('.i-r-diameter').val().trim(),lngth = $(this).find('.i-r-lngth').val().trim(),punit = $(this).find('.i-r-punit').val().trim();
                item.push({id: id,pid:pid, comp: comp, ftre: ftre, sub_ftre: sub_ftre, qnty: qnty,cmpnttype:cmpnttype,forging:forging,diameter:diameter,lngth:lngth,punit:punit});
            }
        });

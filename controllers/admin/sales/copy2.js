                  ...
                  db.query.selectDb('offer_term_conditions_fields', " company=" + req.session.user.cur_company, function (otc) {
                    db.query.selectDb('sales_unit', " company=" + req.session.user.cur_company + "  AND status='active' ", function (su) {
                      db.query.selectDb('enquiry', "id="+req.params.id, function (enqry) {
                       db.query.selectCustomDb("SELECT * FROM prod_specification_category  WHERE show_page = 'subcodification' ORDER BY specification_no" , function (prodcategory) {
                          db.query.selectCustomDb("SELECT otcf.name,otcf.id,otc.message FROM offer_term_conditions_fields otcf INNER JOIN offer_term_conditions otc ON otcf.id=otc.offer_term_conditions INNER JOIN offer o ON o.id = otc.offer WHERE o.enquiry ="+req.params.id, function (oldotc) {
                            ...
                            if(oldofr.length > 0){
                            db.query.selectCustomDb("SELECT p.id,i.id as ofritemid,i.offer as ofrid,i.quantity,i.unit punit,i.rate,i.special_service,i.tax,i.discount,i.profit,i.final_amount,i.tax_amount,i.description,i.special_description,i.hydraulic_body,i.hydraulic_seat,i.pneumatic_seat,i.item_code,i.hydroback_seat,p.name,p.hsn_code,p.specification,p.tax_slabe,p.unit,p.unit_two,p.unit_three,p.code,cmd.size_id,psi.short_code FROM `offer_item` i INNER JOIN item p ON p.id=i.item INNER JOIN component_mapping_details cmd ON p.id=cmd.item LEFT JOIN prod_specification_item psi ON i.pr_parts = psi.id WHERE i.offer='"+oldofr[0].id+"'", function (oitem){  
                              ...
                              res.render('tmpl/offer/new.html', {currency:currency,payment_terms: config.get('payment_terms'), su: su, sa: saf,oitem:oitem, comstate: req.session.company.state, offerno: oldofr[0].offer_no,oldofr:oldofr[0],set: set[0],tax_slab: config.get('tax_slab'),user:req.session.user.name,enquiryid:req.params.id,cusname:cusname[0],enqry:enqry[0],oldotc:otcf,otc:otc,prodcategory:prodcategory,unit:unit,spclsrvc:spclsrvc});
                                });
                              });
                            });
                            }
                           ...
                           /* var amendmentno = (set[0].amendment_pre == 'AI') ? no : ((set[0].amendment_pre == 'FYEAR') ? fyear : set[0].amendment_pre);
                            ...
                            res.render('tmpl/offer/new.html', {currency:currency,payment_terms: config.get('payment_terms'), su: su, sa: sa,oitem:oog, comstate: req.session.company.state, offerno: offerno,amendmentno:amendmentno ,set: set[0],tax_slab: config.get('tax_slab'),user:req.session.user.name,enquiryid:req.params.id,cusname:cusname[0],oldofr:oldofr[0],enqry:enqry[0],otc:otc,oldotc:oldotc,prodcategory:prodcategory,unit:unit,spclsrvc:spclsrvc});
                                }
                              });
                            });
                          });
        ...
        db.query.selectDb('settings', " company=" + req.session.user.cur_company, function (set) {
            db.query.selectCustomDb("SELECT MAX(autono) no FROM offer WHERE company=" + req.session.user.cur_company + " AND fyear='" + fyear + "' ", function (maxno) {
              ...
              /*  console.log("ofrid "+rb.ofrid);
                ...
                var material = rb.mtrial;
                matrerialarr = [];
                matrerialarr = material.split(',');
                console.log(matrerialarr);
                ...
                if(rb.ofrid != '' || newofr.length > 0){
                  ...
                  //  db.query.updateDb('enquiry', {offer:rb.ofrid,offer_status:'od', modification_date: new Date().toMysqlFormat()}, rb.enquiry_id, function (eid) {}); 
                   if(newitm.length > 0){
                     ...
                     db.query.updateDbCustom('offer_item', {offer: upofrid, item: rb.id,quantity: rb.q, unit: rb.u, rate: rb.p, discount: rb.d, final_amount: rb.t,description:rb.gnrldescription,item_code:rb.itmcode,special_description:rb.spcldescription,hydraulic_body:rb.hydbody,hydraulic_seat:rb.hydseat,pneumatic_seat:rb.pneumseat,hydroback_seat:rb.hydbackseat,inspection:rb.inspection,special_service:rb.spclsrvc},"item='"+rb.id+"' AND offer='"+upofrid+"' ", function (oiid) {});
                     db.query.selectCustomDb("SELECT * FROM offer_item WHERE item='"+rb.id+"' AND offer='"+upofrid+"'",function(newofritm){
                       insertupdatematerialdetails(rb,upofrid,newofritm[0].id,matrerialarr,req);
                     });
                    // 
                     ...
                     db.query.insertDb('offer_item', {offer: upofrid, item: rb.id,quantity: rb.q, unit: rb.u, rate: rb.p, discount: rb.d, final_amount: rb.t,description:rb.gnrldescription,item_code:rb.itmcode,special_description:rb.spcldescription,hydraulic_body:rb.hydbody,hydraulic_seat:rb.hydseat,pneumatic_seat:rb.pneumseat,hydroback_seat:rb.hydbackseat,inspection:rb.inspection,special_service:rb.spclsrvc}, function (oiid) {
                     insertmaterialdetails(rb,upofrid,oiid,matrerialarr,req);
                     });
                   }
                  ...
                  db.query.insertDb('offer', {enquiry:rb.enquiry_id,company: req.session.user.cur_company,offer_no:offerno,autono:no, fyear: fyear, offer_note:rb.offer_note, customer: rb.customer, offer_date: bdate, offer_prepared_by:req.session.user.id,offer_validity:rb.offer_validity,total_amount: rb.total_amount,total_quantity:rb.qnty,sales_agent:rb.sales_agent,amount:rb.amount,tax:rb.tax,created_by: req.session.user.id, modified_by: req.session.user.id}, function (pid) {
                    db.query.insertDb('offer_item', {offer: pid, item: rb.id,quantity: rb.q, unit: rb.u, rate: rb.p, discount: rb.d, final_amount: rb.t,description:rb.gnrldescription,item_code:rb.itmcode,special_description:rb.spcldescription,hydraulic_body:rb.hydbody,hydraulic_seat:rb.hydseat,pneumatic_seat:rb.pneumseat,hydroback_seat:rb.hydbackseat,inspection:rb.inspection,special_service:rb.spclsrvc}, function (oiid) {
                  //  db.query.updateDb('enquiry', {offer:pid, modification_date: new Date().toMysqlFormat()}, rb.enquiry_id, function (eid) {}); 
                    insertmaterialdetails(rb,pid,oiid,matrerialarr,req);
                  });
...
function insertmaterialdetails(rb,pid,oiid,matrerialarr,req) {
  console.log("calling from insertmaterialdetails");
  var subarr=[],sub='';
    Object.keys(matrerialarr).forEach(function (k) {
       sub = matrerialarr[k];
       subarr = sub.split(':');
            db.query.insertDb('offer_material', {offer: pid, offer_item: oiid,company:req.session.user.cur_company,prod_specification_category:subarr[0],prod_specification_item:subarr[1]}, function (sid) {});
       
    });
}
function insertupdatematerialdetails(rb,pid,oiid,matrerialarr,req) {
  console.log("calling from insertupdatematerialdetails");
  var subarr=[],sub='';
    Object.keys(matrerialarr).forEach(function (k) {
       sub = matrerialarr[k];
       subarr = sub.split(':');

            db.query.updateDbCustom('offer_material', {offer: pid, offer_item: oiid,company:req.session.user.cur_company,prod_specification_category:subarr[0],prod_specification_item:subarr[1]},"offer='"+pid+"' AND offer_item='"+oiid+"' AND prod_specification_category='"+subarr[0]+"' ", function (sid) {});
            ...
            for (var i = 0; i < data[0].length; i++) {
             ...
             var pstatus = '<a class="btn btn-danger btn-trans waves-effect w-md waves-danger m-b-5">expired</a>';
                if (data[0][i].item_mapping_status == 'yes') {
                   ...
                   action +=  '<button type="button" class="btn btn-secondary dropdown-toggle waves-effect btn-success" data-toggle="dropdown" aria-expanded="false"> Action <span class="caret"></span> </button>'
                   action += '<div class="dropdown-menu">'
                   action += '<a data-id="' + data[0][i].itmid + '" data-oid="' + data[0][i].id + '" data-subitm="' + data[0][i].sub_item + '" data-oiid="' + data[0][i].oiid + '" class="dropdown-item s-l-im" title="Item_mapping">Mapping</a>'
                   action += '</div>'
                   action += '</div>'
                   // pstatus = '<a class="btn btn-success btn-trans waves-effect w-md waves-success m-b-5">Active</a>';
...
router.get('/getsubmaterialcode', function (req, res) {
          var pid = req.query.pid;
          var prodid = pid.replace(/"/g,"");
        var sql="SELECT * FROM offer_material  where  offer_item ='"+prodid+"'";
          db.query.selectCustomDb(sql , function (itemcode) {    
           res.end(JSON.stringify(itemcode))
      });
});

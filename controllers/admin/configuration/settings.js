...
router.post('/generalsave', function (req, res) {
    ...
    db.query.deleteDb('offer_term_conditions_fields', " company=" + req.session.user.cur_company, function (uid) {});
    for (var k in rb.addTerm) {
        db.query.insertDb('offer_term_conditions_fields', {name: rb.addTerm[k].name,msg: rb.addTerm[k].msg, company: req.session.user.cur_company, created_by: req.session.user.id, modify_by: req.session.user.id}, function (cid) {});
    }
...
router.get('/currency', function (req, res) {
    db.query.selectDb('currency', " company=" + req.session.user.cur_company, function (row) {
                res.render('tmpl/setting/currency.html', {row:row});
            });
});
router.post('/currencysave', function (req, res) {
    var rb = req.body;
    db.query.deleteDb('currency', " company=" + req.session.user.cur_company, function (uid) {});
    for (var k in rb.addfield) {
        db.query.insertDb('currency', {company: req.session.user.cur_company,name: rb.addfield[k].name,val: rb.addfield[k].val, created_by: req.session.user.id, modify_by: req.session.user.id}, function (cid) {});
    }
    generateLogFile({user: req.session.user.id, type: 'currency updated', time: new Date(), refer_id: req.session.user.cur_company});
    res.end(JSON.stringify({'code': '1', 'msg': 'Success'}));
});

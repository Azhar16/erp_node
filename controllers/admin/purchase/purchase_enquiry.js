         ...
         jsonfile.readFile('config/default.json', function (err, currency) {
           db.query.selectDb('sales_additional_fields', " company=" + req.session.user.cur_company, function (sa) {
             db.query.selectDb('sales_unit', " company=" + req.session.user.cur_company + "  AND status='active' ", function (su) {
                res.render('tmpl/purchase_enquiry/new.html', {currency:currency,payment_terms: config.get('payment_terms'), su: su, sa: sa, comstate: req.session.company.state,tax_slab: config.get('tax_slab'),user:req.session.user.name,cus:cus,ship:ship});                   
          });
        });
      });
    });
...
router.get('/autonum/:v', function (req, res) {
    ...
    db.query.selectDb('settings', " company=" + req.session.user.cur_company, function (set) {
       ...
       var no = (maxno[0].no + 1).toString().padStart(5, '0');
        var enquiryno = (set[0].penquiry_pre == 'AI') ? no : ((set[0].penquiry_pre == 'FYEAR') ? fyear : set[0].penquiry_pre);
        //enquiryno += (set[0].enquiry_pre != '') ? set[0].enquiry_div : '';
        enquiryno += otype;
        enquiryno += (otype != '') ? set[0].penquiry_div : '';
        enquiryno += (set[0].penquiry_cen == 'AI') ? no : ((set[0].penquiry_cen == 'FYEAR') ? fyear : set[0].penquiry_cen);
        enquiryno += (set[0].penquiry_cen != '') ? set[0].penquiry_div : '';
        enquiryno += (set[0].penquiry_su == 'AI') ? no : ((set[0].penquiry_su == 'FYEAR') ? fyear : set[0].penquiry_su);
        res.end(JSON.stringify(enquiryno));
    });
  });
});
...
router.get('/docview/:id', function (req, res) {
        db.query.selectDb('purchase_enquiry_doc', " purchase_enquiry=" + req.params.id, function (row) {
        res.render('tmpl/purchase_enquiry/docview.html', {row: row,sid:req.params.id});
    });
});
router.get('/docdelete/:id', function (req, res) {
    db.query.deleteDb('purchase_enquiry_doc', "purchase_enquiry=" + req.params.id, function (cid) {
       generateLogFile({user: req.session.user.id, type: 'Document delete', time: new Date(), refer: req.params.id});
        res.end(JSON.stringify({'code': '1', 'msg': 'Success'}));
    });
});
router.post('/save', function (req, res) {
    ...
    var expected_delivery_date = moment(rb.expected_delivery_date + ' 12:00:00').utc().format("YYYY-MM-DD HH:mm:ss");
    var ven = [];
    ven = rb.vendor;
    var fnlven = ven.join();
    db.query.selectCustomDb("SELECT MAX(autono) no FROM purchase_enquiry WHERE company=" + req.session.user.cur_company + " AND fyear='" + fyear + "' ", function (maxno) {
    ...
    if (typeof rb.sid !== 'undefined') {
        db.query.updateDb('purchase_enquiry', {enquiry_note: rb.enquiry_note,enquiry_terms:rb.enquiry_terms_note, vendor: fnlven,shipping:rb.shipping, date: bdate,expected_delivery_date:expected_delivery_date,submission_date:submission_date,total_amount: rb.total_amount,modify_by:req.session.user.id,modification_date: new Date().toMysqlFormat()}, rb.sid, function (pid) {
            ...
            res.end(JSON.stringify({'code': rb.sid, 'msg': 'Success'}));
                ...
                db.query.insertDb('purchase_enquiry', {company: req.session.user.cur_company,enquiry_no:rb.enquiry_no,autono:no, fyear: fyear, enquiry_note: rb.enquiry_note,enquiry_terms:rb.enquiry_terms_note, vendor: fnlven,shipping:rb.shipping, date: bdate,expected_delivery_date:expected_delivery_date,submission_date:submission_date,total_amount: rb.total_amount,created_by: req.session.user.id}, function (pid) {
...
router.post('/docsave', function (req, res) {
    ...
    form.uploadDir = config.get('purchase_enquiry_doc');
    form.on('file', function (field, file) {
        fs.renameSync(file.path, path.join(form.uploadDir, ctime + file.name));
    });
    form.parse(req, function (err, fields, files) {
        var sql = "SELECT * from  purchase_enquiry_doc  WHERE item='" + fields.did +"' AND  purchase_enquiry='" + fields.sid +"'";
        db.query.selectCustomDb(sql, function (row) {

       var docname = '';
          if (typeof (files.i_r_doc) != 'undefined' ){
              docname = ctime + files.i_r_doc.name;
           }

           if(row.length > 0){
            if(docname.length >0 || docname != ''){
             db.query.updateDb('purchase_enquiry_doc', {doc:docname}, fields.sid, function (pid) {});
            }
           }else{
           if(docname.length >0 || docname != ''){
           db.query.insertDb('purchase_enquiry_doc', {doc:docname,purchase_enquiry:fields.sid,item:fields.did}, function (did) {});
            }
        }
         //    res.end(JSON.stringify({'code': 1, 'msg': 'Success'}));
    });

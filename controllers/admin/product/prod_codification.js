...
router.get('/itemmapping/:id', function (req, res) {
  db.query.selectDb('hydro_test', "status<>'delete' AND company=" + req.session.user.cur_company, function (hydro) {
    db.query.selectDb('component_category_name', "product_type = 'finished' AND status<>'delete' AND company=" + req.session.user.cur_company, function (row) {
       ...
       // console.log("ends_out "+proddi[0].ends_out);
...
router.post('/save', function (req, res) {
    var rb = req.body;
     ...
     if (rb.prodid != undefined) {
            ...
            db.query.insertDb('product_codification', {prod_specification_category:rb.category[k].specificid,prod_specification_item:rb.category[k].item_name,product_item_id:rb.prodid,created_by:req.session.user.id}, function (pid) {});
        ...
        if (stat == true) {    
                ...
                db.query.selectDb('component_mapping', " item=" + rb.itmid, function (cmapping) {
                  for(var k in cmapping){
                    db.query.insertDb('component_mapping', {type:'finished',item:itmid,company: req.session.user.cur_company,component: cmapping[k].component, component_feature: cmapping[k].component_feature,component_sub_feature: cmapping[k].component_sub_feature, prod_quantity: cmapping[k].prod_quantity,drawing_file:cmapping[k].drawing_file,drawing_no:cmapping[k].drawing_no,component_type:cmapping[k].component_type,forging:cmapping[k].forging,diameter:cmapping[k].diameter,length:cmapping[k].length,punit:cmapping[k].punit,width:cmapping[k].width,thickness:cmapping[k].thickness, created_by: req.session.user.id},function (compmap) {});
                  }
                });
                res.end(JSON.stringify({'code': '1', 'msg': 'Success'}));
                });
...
router.post('/itemmappingsave', function (req, res) {
    ...
    if(typeof row[0] != 'undefined'){
      db.query.updateDbCustom('component_mapping_details', {size_id:rb.size_id, size: rb.product_size, type: rb.product_type, class: rb.product_class, figure: rb.product_figure, ends: rb.product_ends,ends_out: rb.product_ends_out,joint:rb.product_joint,hydraulic_body: rb.product_hydraulic_body, hydraulic_seat: rb.product_hydraulic_seat,pneumatic_seat:rb.product_pneumatic_seat,hydroback_seat:rb.product_hydroback_seat, unit_rate: rb.product_unit_rate, category: rb.product_category,modify_by: req.session.user.id}, "item=" + rb.compmapid, function (sid) {});
    ...
    db.query.insertDb('component_mapping_details', {item:rb.compmapid,company: req.session.user.cur_company,size_id:rb.size_id, size: rb.product_size, type: rb.product_type, class: rb.product_class, figure: rb.product_figure, ends: rb.product_ends,ends_out: rb.product_ends_out,joint:rb.product_joint,hydraulic_body: rb.product_hydraulic_body, hydraulic_seat: rb.product_hydraulic_seat,pneumatic_seat:rb.product_pneumatic_seat,hydroback_seat:rb.product_hydroback_seat, unit_rate: rb.product_unit_rate, category: rb.product_category, created_by: req.session.user.id}, function (compmapdtl) {});
    ...
    if(rb.item[k].pid != 0){
      console.log("pid "+rb.item[k].pid);
        db.query.updateDbCustom('component_mapping', {component: rb.item[k].comp, component_feature: rb.item[k].ftre,component_sub_feature: rb.item[k].sub_ftre, prod_quantity: rb.item[k].qnty,component_type:rb.item[k].cmpnttype,forging:rb.item[k].forging,diameter:rb.item[k].diameter,length:rb.item[k].lngth,punit:rb.item[k].punit,width:rb.item[k].wdth,thickness:rb.item[k].thickness, created_by: req.session.user.id, modify_by: req.session.user.id}," id=" + rb.item[k].pid,function (compmap) {
                    generateLogFile({user: req.session.user.id, type: 'component mapping updated', time: new Date(), refer_id: compmap});
                });
      }
      else{
        db.query.insertDb('component_mapping', {type:'finished',item:rb.compmapid,company: req.session.user.cur_company, component: rb.item[k].comp, component_feature: rb.item[k].ftre,component_sub_feature: rb.item[k].sub_ftre, prod_quantity: rb.item[k].qnty,component_type:rb.item[k].cmpnttype,forging:rb.item[k].forging,diameter:rb.item[k].diameter,length:rb.item[k].lngth,punit:rb.item[k].punit,width:rb.item[k].wdth,thickness:rb.item[k].thickness, created_by: req.session.user.id, modify_by: req.session.user.id}, function (compmap) {

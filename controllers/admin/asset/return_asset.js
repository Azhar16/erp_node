...
router.get('/', function (req, res) {
    res.render('tmpl/return_asset/list.html');
...
router.post('/ajaxget', function (req, res) {
    ...
    db.query.getReturnAssetVen({company: req.session.user.cur_company, ttype: 'customer', val: rb.search.value, start: rb.start, tot: rb.length}, function (data) {
        if (data[0].length > 0) {
            ...
            for (var i = 0; i < data[0].length; i++) {
                ...
                var action = '<button data-id="' + data[0][i].id + '" class="btn btn-icon waves-effect waves-light btn-primary m-b-5 s-l-v" title="View"> <i class="fa fa-eye"></i> </button>&nbsp;';
                    ...
                    arrObj.data.push([data[0][i].id,data[0][i].return_date,data[0][i].name, action]);
            }
...
router.get('/new', function (req, res) {       
    res.render('tmpl/return_asset/new.html');      
});

router.get('/edit/:id', function (req, res) {
    db.query.getReturnAssetEdit({id: req.params.id, company: req.session.user.cur_company}, function (row) {
        res.render('tmpl/return_asset/edit.html',{row: row});
    });
});

router.get('/delete/:id', function (req, res) {
    db.query.selectDb('return_asset', " id=" + req.params.id, function (sl) {
        db.query.deleteDb('return_asset', " id=" + req.params.id, function (cid) {
          db.query.deleteDb('return_asset_item', " return_asset=" + req.params.id, function (did) {
...
router.post('/save', function (req, res) {
    ...
    var return_date = moment(rb.return_date + ' 12:00:00').utc().format("YYYY-MM-DD HH:mm:ss");
 if (typeof rb.did !== 'undefined') {
                db.query.updateDb('return_asset', {partner:rb.partner,return_date:return_date,remark:rb.return_remarks, modify_by: req.session.user.id,modification_date: new Date().toMysqlFormat()}, rb.did, function (eid) {
                  db.query.deleteDb('return_asset_item', " return_asset=" + rb.did, function (cid) {});
                    insertSalesDetail(rb, rb.did, req);
                    generateLogFile({user: req.session.user.id, type: 'return asset modification', time: new Date(), refer_id: rb.planid});
                    res.end(JSON.stringify({'code': rb.did, 'msg': 'Success'}));
                });
            }

else{
    db.query.insertDb('return_asset', {company:req.session.user.cur_company,partner:rb.partner,return_date:return_date,remark:rb.return_remarks, created_by: req.session.user.id}, function (pid) {
...
function insertSalesDetail(rb, pid,req) {
    Object.keys(rb.item).forEach(function (k) {
        db.query.insertDb('return_asset_item', {return_asset: pid, asset: rb.item[k].id, quantity: rb.item[k].q}, function (sid) {});
    });

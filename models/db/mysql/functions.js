...
asyncinserCustomtDb: function (query) {
    var _this = this;
    console.log(query);
    return new Promise(function (result) {
        _this.pconnection.query(query, function (err, res) {
            if (err)
                throw err;
            result(res.insertId);
            ;
...
getSalesVen: function (data, cb) {
    ...
    var cnd = "",_this=this;
    if (typeof data.company != 'undefined') {
        cnd += " AND o.submit_status='full' AND o.company=" + data.company;
    }
...
getEnquiryVen: function (data, cb) {
    ...
    if (typeof data.val != 'undefined') {
        cnd += " AND (e.enquiry_validity LIKE '%" + data.val + "%'  OR e.enquiry_status LIKE '%" + data.val + "%' OR c.name LIKE '%" + data.val + "%' OR c.company_name LIKE '%" + data.val + "%' OR e.enquiry_no LIKE '%" + data.val + "%' OR e.ref_no LIKE '%" + data.val + "%') ";
    }
    var sql = "SELECT e.id,e.enquiry_status,e.enquiry_validity,e.enquiry_no,e.ref_no,c.name,c.company_name FROM `enquiry` e INNER JOIN customer c ON c.id=e.customer where 1"+cnd+" ORDER BY e.id DESC LIMIT "+data.start+","+data.tot;
    this.selectCustomDb(sql, function (row) {
...
ckDuplicateEnquiry:function(data,cb){
    var cnd='';
    if(typeof data.id!='undefined')
        cnd=' AND id<>'+data.id;
    var sql = "SELECT id FROM `enquiry` WHERE status='active' and ref_no='" + data.code.trim() + "'  "+ cnd;
    console.log(sql);
    this.selectCustomDb(sql, function (row) {
        if(row.length>0)
        return cb(false);
...
getOfferEdit:function(data,cb){
    var arr={},_this=this;
    var sql = "SELECT id,DATE_FORMAT(offer_date,'%m/%d/%Y') offer_date,amendment_no,amendment_date,offer_status,customer,offer_no,offer_note,offer_validity,total_amount,total_quantity,offer_term_note,sales_agent,tax,amount,ld_clause,currency,currency_value FROM `offer` WHERE id='" + data.id + "' ";
    ...
    this.selectCustomDb(sql, function (row) {
        row.forEach(async function (sa) {
            ...
            arr.item=await _this.asyncselectCustomDb("SELECT p.id,i.quantity,i.id as ofritemid,i.standard,i.enq_serial_no,i.discount_numeric,i.unit punit,i.rate,i.tax,i.discount,i.final_amount,i.tax_amount,i.description,i.trim_parts,i.pr_parts,i.stem,i.special_description,i.hydraulic_body,i.hydraulic_seat,i.pneumatic_seat,i.item_code,i.hydroback_seat,i.inspection,p.name,p.hsn_code,p.specification,p.tax_slabe,p.unit,p.unit_two,p.unit_three,p.code,cmd.size_id,mm.short_code FROM `offer_item` i INNER JOIN item p ON p.id=i.item INNER JOIN component_mapping_details cmd ON p.id=cmd.item LEFT JOIN material_master mm ON mm.id = i.pr_parts WHERE i.offer='" + data.id + "'");
...
getOfferproductEdit:function(data,cb){
    var arr={},_this=this;
    var sql = "SELECT id,DATE_FORMAT(offer_date,'%m/%d/%Y') offer_date,amendment_no,amendment_date,offer_status,customer,offer_no,offer_note,offer_validity,total_amount,offer_term_note,tax FROM `offer` WHERE id='" + data.id + "' ";
   // console.log("SELECT id,DATE_FORMAT(offer_date,'%m/%d/%Y') offer_date,amendment_no,amendment_date,offer_status,customer,offer_no,offer_note,offer_validity,total_amount,offer_term_note FROM `offer` WHERE id='" + data.id + "' ");
    this.selectCustomDb(sql, function (row) {
        row.forEach(async function (sa) {
            arr.sa=sa;
            ...
            arr.item=await _this.asyncselectCustomDb("SELECT p.id,i.quantity,i.id as ofritemid,i.standard,i.enq_serial_no,i.discount_numeric,i.unit punit,i.rate,i.tax,i.discount,i.final_amount,i.tax_amount,i.description,i.trim_parts,i.pr_parts,i.stem,i.special_description,i.hydraulic_body,i.hydraulic_seat,i.pneumatic_seat,i.item_code,i.hydroback_seat,p.name,p.hsn_code,p.specification,p.tax_slabe,p.unit,p.unit_two,p.unit_three,p.code,cmd.size_id,mm.short_code FROM `offer_item` i INNER JOIN item p ON p.id=i.item INNER JOIN component_mapping_details cmd ON p.id=cmd.item LEFT JOIN material_master mm ON mm.id = i.pr_parts WHERE i.offer='" + data.id + "'");
            arr.newitem=await _this.asyncselectCustomDb("SELECT p.id,i.quantity,i.id as ofritemid,i.standard,i.enq_serial_no,i.loa_serial_no,i.po_serial_no,i.discount_numeric,i.unit punit,i.rate,i.tax,i.discount,i.final_amount,i.tax_amount,i.description,i.trim_parts,i.pr_parts,i.stem,i.special_description,i.hydraulic_body,i.hydraulic_seat,i.pneumatic_seat,i.item_code,i.hydroback_seat,p.name,p.hsn_code,p.specification,p.tax_slabe,p.unit,p.unit_two,p.unit_three,p.code,cmd.size_id,mm.short_code FROM `workorder_item` i INNER JOIN item p ON p.id=i.item INNER JOIN component_mapping_details cmd ON p.id=cmd.item LEFT JOIN material_master mm ON mm.id = i.pr_parts WHERE i.offer='" + data.id + "' AND i.workorder='"+data.woid+"'");
...
getTechnoCommercialworkorder:function(data,cb){
   if (typeof data.id != 'undefined' && data.id != '') {
    var cnd = '', _this = this ,arr = [],item = [];
    var sql = "SELECT  * FROM workorder_technocommercial WHERE workorder='" + data.id + "' ";
    this.selectCustomDb(sql, function (row) {
    var item = [], pending = row.length;
        if (row.length > 0) {
        row.forEach(async function (sa) {
            arr.sa=sa;
            arr.sp=await _this.asyncselectCustomDb("SELECT * FROM `acknowledgement_shipping`  WHERE  workorder_technocommercial='" + sa.id + "' ");
            item.push(arr);
                    if (0 === --pending) {
                        return cb(item);
                    }
...
reportofferDet: function (data, cb) {
    if (typeof data.frm_date != 'undefined' && data.frm_date != '' && typeof data.to_date != 'undefined' && data.to_date != '') {
        var sql = "SELECT o.id,oi.id as oiid,o.offer_no,DATE_FORMAT(o.offer_date,'%d, %b %Y') offer_date,c.company_name,c.address,c.state,c.zip,c.city,c.gst,o.offer_term_note,o.total_amount,o.offer_note,oi.quantity,oi.unit,oi.rate,oi.discount,oi.discount_numeric,oi.tax_amount,oi.tax,oi.final_amount,oi.item_code,oi.special_description,oi.trim_parts,mm.code as trim_parts,mmp.code as pr_parts,mmp.short_code,i.code,i.specification,i.name,i.hsn_code,cmd.size_id,cmd.size,cmd.type,cmd.class,cmd.ends,cmd.ends_out,cmd.joint,ins.label as inspection FROM `offer` o INNER JOIN customer c on c.id = o.customer  INNER JOIN offer_item oi ON oi.offer = o.id INNER JOIN item i ON i.id = oi.item LEFT JOIN component_mapping_details cmd ON cmd.item = oi.item LEFT JOIN material_master mm ON mm.id = oi.trim_parts LEFT JOIN material_master mmp ON mmp.id = oi.pr_parts LEFT JOIN inspection ins ON oi.inspection = ins.id WHERE o.status <>'delete' AND o.submit_status='full' AND o.offer_date>'" + data.frm_date + "' AND o.offer_date<'" + data.to_date + "' ORDER BY o.id";
        this.selectCustomDb(sql, function (row) {
            return cb(row);
        });
    } else {
...
getprodcodedetails: function(data,cb){
    ...
    this.selectCustomDb(sql, function (row) {
         ...
         row.forEach(async function (prodcode) {
                    ...
                    if(specificationcategoryitem[0].specification_name == 'ENDS_OUT'){
                        arr.ends_out = specificationitem[0].code;
                    }
                    ...
                    if(specificationcategoryitem[0].specification_name == 'SUPPLEMENTARY'){
                        if(specificationitem[0].short_code != ''){
                           arr.supli = specificationitem[0].short_code; 
                        }
                        else{
                            arr.supli = '';
                        }
...
getPurchaseOrderEdit:function(data,cb){
    var arr={},_this=this;
    var sql = "SELECT po.id,po.po_no as pono,DATE_FORMAT(po.po_date,'%m/%d/%Y') date,DATE_FORMAT(po.delivery_date,'%m/%d/%Y') delivery_date,po.quotation_no,po.price_competitive,po.vendor,po.purchase_enquiry as enquiry,po.total_amount,po.note,pe.enquiry_no FROM purchase_order po LEFT JOIN purchase_enquiry pe ON po.purchase_enquiry = pe.id WHERE po.id='" + data.id + "' ";
    this.selectCustomDb(sql, function (row) {
        row.forEach(async function (sa) {
            arr.sa=sa;
            arr.cus=await _this.asyncselectCustomDb("SELECT * FROM `customer`  WHERE id='" + sa.vendor + "' ");
            ...
            arr.saf=await _this.asyncselectCustomDb("SELECT paf.account,paf.name,paf.id,paf.amount,pa.tax,pa.tax_amount,pa.amount samount FROM purchase_additional_fields paf LEFT JOIN purchase_order_additional pa ON paf.id=pa.purchase_order_additional AND pa.purchase_order='" + sa.id + "'");
            arr.item=await _this.asyncselectCustomDb("SELECT p.id,i.quantity,i.unit punit,i.rate,i.tax,i.final_amount,i.tax_amount,p.code,p.hsn_code,p.specification,p.tax_slabe,p.unit,p.unit_two,p.unit_three FROM `purchase_order_item` i INNER JOIN item p ON p.id=i.item WHERE i.purchase_order='" + sa.id + "'");
            arr.otc=await _this.asyncselectCustomDb("SELECT ptcf.name,ptcf.id,ptc.message FROM po_term_conditions_fields ptcf LEFT JOIN po_term_conditions ptc ON ptcf.id=ptc.po_term_conditions_fields WHERE ptc.purchase_order='" + sa.id + "'");
            return cb(arr);
        });
    });
},
...
getofferMappingVen:function(data,cb){
    ...
    var sql = "SELECT o.id,i.id as itmid,oi.id as oiid,i.code,cmd.size_id,i.description,oi.description as desp,oi.item_mapping_status FROM offer o INNER JOIN offer_item oi ON o.id=oi.offer INNER JOIN item i ON oi.item=i.id INNER JOIN component_mapping_details cmd ON cmd.item=i.id WHERE 1"+cnd+" ORDER BY oi.id DESC LIMIT "+data.start+","+data.tot;
    //console.log(sql);
    this.selectCustomDb(sql, function (row) {
...
newWoMappingdetailsByItemId:function(data,cb){
    var arr={},_this=this;
    var sql = "SELECT wm.id as wmid,wm.material,cm.id,cm.item,cm.prod_quantity,ccn.id as ccnid,ccn.component_name,pf.id as pfid,pf.name as pfname ,psf.id as psfid,psf.name as psfname FROM workorder_mapping wm INNER JOIN component_mapping cm ON wm.component_mapping = cm.id INNER JOIN component_category_name ccn ON cm.component = ccn.id INNER JOIN product_feature pf ON cm.component_feature = pf.id LEFT JOIN product_sub_feature psf ON psf.id = cm.component_sub_feature WHERE wm.item='"+data.itm+"' AND wm.workorder='"+data.woid+"' AND wm.workorder_item='"+data.wiid+"'";
    console.log(sql);
    this.selectCustomDb(sql, function (row) {
      return cb(row);
  });
...
getWoMappingdetailsByItemId:function(data,cb){
    var arr={},_this=this;
    var sql = "SELECT cm.id,cm.item,cm.prod_quantity,ccn.id as ccnid,ccn.component_name,pf.id as pfid,pf.name as pfname ,psf.id as psfid,psf.name as psfname,wi.trim_parts,wi.pr_parts,wi.stem FROM component_mapping cm INNER JOIN component_category_name ccn ON cm.component = ccn.id INNER JOIN product_feature pf ON cm.component_feature = pf.id LEFT JOIN product_sub_feature psf ON psf.id = cm.component_sub_feature INNER JOIN workorder_item wi ON wi.item = cm.item   WHERE cm.item='" + data.itm + "' AND wi.id = '"+data.wiid+"'";
    console.log(sql);
    this.selectCustomDb(sql, function (row) {
      return cb(row);
  });
...
getsubcodificationVen: function(data, cb) {
  ...
  reportallOpeningsDet: function(data, cb){
       if (typeof data.fyear != 'undefined' ) {
        var cnd = '', _this = this ,item = [];
        var sql = "SELECT ai.id,ai.fyear,ai.date,DATE_FORMAT(ai.date,'%d, %b %Y') aidate,ait.quantity,ait.rate,ait.shift,a.asset_name,att.useful_life,ar.wdv,ar.slm,ait.sale,sai.rate as samount,DATE_FORMAT(sa.date,'%d, %b %Y') sadate,a.asset_type FROM asset_info ai INNER JOIN asset_item ait ON ai.id = ait.asset_info INNER JOIN asset a ON ait.asset = a.id INNER JOIN assettype att ON a.asset_type=att.id INNER JOIN assetrate ar ON att.useful_life = ar.useful_life LEFT JOIN sale_asset_item sai ON sai.id = ait.sale_asset_item LEFT JOIN sale_asset sa ON sa.id = sai.sale_asset WHERE ai.fyear='1718'";
        this.selectCustomDb(sql, function (row) {
                var item = [], pending = row.length;
                if (row.length > 0) {
                    row.forEach(async function (opening) {
                        var arr = opening;
                        assetfy = opening.fyear;
                        var astYear = assetfy.toString().substring(0,2);
                        var crntyear = data.fyear.toString().substring(0,2);
                        var result = (crntyear - astYear);
                        var remaning_life = (opening.useful_life - result);
                        var sublife = (opening.useful_life - remaning_life)

                        console.log("remaning_life "+sublife);

                            var depval = 0;
                            var deptval = 0;
                            var orate = 0;
                            var curdeprate = 0;
                        var deprate = await _this.asyncselectCustomDb("SELECT * FROM depreciation_rate WHERE assettype='" + opening.asset_type + "' ");
                        for(var i = 0; i < sublife; i++) {
                            console.log("type "+typeof i);
                            var ilife = (parseInt(i) +1);
                            
                            if(i == 0){
                            orate  = opening.rate;
                            depval = opening.rate * deprate[i].company_act / 100;
                            }
                            deptval = (orate - depval);
                            if(i != 0){
                                console.log("1 "+orate);
                                orate = deptval;
                                depval = orate* deprate[i].company_act / 100;
                                deptval = (orate - depval);
                            }
                            curdeprate = deprate[i].company_act;
                            console.log("orate "+i+"=="+orate);
                            console.log("depval "+i+"=="+depval);
                            console.log("deptval "+i+"=="+deptval);
                            


                            
                            console.log("deprate "+i+"=="+deprate[i].company_act);
                            //console.log("depval "+depval);
                        }
                        arr.prevrate = orate;
                        arr.depval = depval;
                        arr.nextrate = deptval;
                        arr.curdeprate = curdeprate;
                        arr.remaning_life = remaning_life;
...
reportallpoDetByDeliveryDate: function (data, cb) {
    if (typeof data.fdate != 'undefined' && data.fdate != '' && typeof data.cdate != 'undefined' && data.cdate != '') {
       var cnd = '', _this = this,order_by=" c.company_name ASC,UNIX_TIMESTAMP(po_date) ASC ";
      //  var sql = "SELECT e.enquiry_no,c.company_name,e.enquiry_validity,c.company_name, DATE_FORMAT(e.query_date,'%d, %b %Y') query_date,e.ref_no,c.company_name,sa.name FROM enquiry e INNER JOIN customer c ON e.customer = c.id INNER JOIN sales_agent sa ON sa.id = e.sales_agent WHERE e.status <>'delete' AND e.query_date>'" + data.frm_date + "' AND e.query_date<'" + data.to_date + "' ORDER BY e.id";
        if (data.status != 'all')
            cnd += " AND po_status='" + data.status + "' ";
        var sql = "SELECT po.po_no,c.company_name, DATE_FORMAT(po.po_date,'%d, %b %Y') po_date,c.company_name,po.total_amount,pe.enquiry_no FROM purchase_order po INNER JOIN customer c on c.id=po.vendor LEFT JOIN purchase_enquiry pe ON pe.id = po.purchase_enquiry WHERE po.status <>'delete' AND po.po_date>'" + data.fdate + "' AND po.po_date<'" + data.tdate + "' "+ cnd +" ORDER BY "+order_by;
        console.log(sql);
        this.selectCustomDb(sql, function (row) {
           return cb(row); 
        }); 
...
getprodilist: function (data, cb) {
    ...
    if (typeof data.val != 'undefined') {
        cnd += " AND ( cmd.size_id LIKE '%" + data.val + "%') ";
    }
    var sql = "SELECT pc.id,i.code as product_item_id,i.id as itemid,cmd.size_id FROM product_codification pc INNER JOIN item i ON pc.product_item_id = i.id INNER JOIN component_mapping_details cmd ON cmd.item=i.id  where 1"+cnd+" GROUP BY pc.product_item_id ORDER BY cmd.size_id ASC LIMIT "+data.start+","+data.tot;
    //console.log(sql);
    this.selectCustomDb(sql, function (row) {
        sql = "SELECT count(DISTINCT (pc.product_item_id)) tot FROM  product_codification pc INNER JOIN item i ON pc.product_item_id = i.id INNER JOIN component_mapping_details cmd ON cmd.item=i.id where 1"+cnd;
 ...
 getspecialserviceVen: function (data, cb) {
    var cnd = "",_this=this;
    if (typeof data.company != 'undefined') {
        cnd += " AND company=" + data.company;
    }
    if (typeof data.val != 'undefined') {
        cnd += " AND (service_name LIKE '%" + data.val + "%' OR service_no LIKE '%" + data.val + "%' ) ";
    }
    var sql = "SELECT id,service_name,service_no FROM special_service   where 1"+cnd+" ORDER BY service_no DESC LIMIT "+data.start+","+data.tot;
    //console.log(sql);
    this.selectCustomDb(sql, function (row) {
        sql = "SELECT count(id) tot FROM special_service where 1"+cnd;
...
getSubServiceVen: function (data, cb) {
    var cnd = "",_this=this;
    if (typeof data.special_service != 'undefined') {
        cnd += " AND special_service=" + data.special_service;
    }
    if (typeof data.val != 'undefined') {
        cnd += " AND (name LIKE '%" + data.val + "%' OR no LIKE '%" + data.val + "%' ) ";
    }
    var sql = "SELECT id,name,no FROM sub_special_service   where 1"+cnd+" ORDER BY no DESC LIMIT "+data.start+","+data.tot;
    console.log(sql);
    this.selectCustomDb(sql, function (row) {
        sql = "SELECT count(id) tot FROM `sub_special_service`  where 1"+cnd;
...
getmaterialVen: function (data, cb) {
    var cnd = "",_this=this;
    if (typeof data.componentid != 'undefined') {
        cnd += " AND component_category_name=" + data.componentid;
    }
    if (typeof data.val != 'undefined') {
        cnd += " AND (name LIKE '%" + data.val + "%' OR code LIKE '%" + data.val + "%' OR short_code LIKE '%" + data.val + "%' OR showing LIKE '%" + data.val + "%' ) ";
    }
    var sql = "SELECT * FROM material_master where 1"+cnd+" ORDER BY id DESC LIMIT "+data.start+","+data.tot;
    //console.log(sql);
    this.selectCustomDb(sql, function (row) {
        sql = "SELECT count(id) tot FROM material_master  where 1"+cnd;
...
insertofferitemdetails: function (data, cb) {
    if (typeof data.ofr != 'undefined' && data.ofr != '' && typeof data.woid != 'undefined' && data.woid != '') {
        var cnd = '', _this = this ,item = [];
        var sql = "SELECT * FROM offer_item WHERE offer='" + data.ofr + "' ";
        this.selectCustomDb(sql, function (row) {
                var item = [], pending = row.length;
                if (row.length > 0) {
                    row.forEach(async function (ofritm) {
                        var arr = ofritm;
                        var oiid = await _this.asyncinserCustomtDb("INSERT INTO workorder_item SET workorder = '"+data.woid+"',offer='"+data.ofr+"', item='"+ofritm.item+"',quantity='"+ofritm.quantity+"',tax='"+ofritm.tax+"',tax_amount='"+ofritm.tax_amount+"',unit='"+ofritm.unit+"',rate='"+ofritm.rate+"',discount='"+ofritm.discount+"',final_amount='"+ofritm.final_amount+"',description='"+ofritm.description+"',pr_parts='"+ofritm.pr_parts+"',trim_parts='"+ofritm.trim_parts+"',stem='"+ofritm.stem+"',item_code='"+ofritm.item_code+"',special_description='"+ofritm.special_description+"',hydraulic_body='"+ofritm.hydraulic_body+"',hydraulic_seat='"+ofritm.hydraulic_seat+"',pneumatic_seat='"+ofritm.pneumatic_seat+"',hydroback_seat='"+ofritm.hydroback_seat+"',inspection='"+ofritm.inspection+"',standard='"+ofritm.standard+"',enq_serial_no='"+ofritm.enq_serial_no+"',discount_numeric= '"+ofritm.discount_numeric+"' ");
                        console.log("oiiid "+oiid);
                        var ofrspcl = await _this.asyncselectCustomDb("SELECT * FROM offer_special_service WHERE offer='" + data.ofr + "' AND offer_item='"+ofritm.id+"'");
                        if(ofrspcl.length > 0){
                        for(var i = 0; i < ofrspcl.length; i++) {
                            var sid = await _this.asyncinserCustomtDb("INSERT INTO workorder_special_service SET workorder= '"+data.woid+"', workorder_item='"+oiid+"',company='"+data.company+"',special_service='"+ofrspcl[i].special_service+"',sub_special_service='"+ofrspcl[i].sub_special_service+"' ");
                             
                        }
                    }
                    var matmaster = await _this.asyncselectCustomDb("SELECT * FROM component_sub_mapping WHERE offer='" + data.ofr + "' AND offer_item='"+ofritm.id+"'");
                        if(matmaster.length > 0){
                        for(var j = 0; j < matmaster.length; j++) {
                            var pid = await _this.asyncinserCustomtDb("INSERT INTO workorder_mapping SET workorder='"+data.woid+"', workorder_item='"+oiid+"',company='"+data.company+"',item='"+matmaster[j].item+"',material='"+matmaster[j].material+"',component_mapping='"+matmaster[j].component_mapping+"' ");
                        }
                    }
                    item.push(arr);
                    if (0 === --pending) {
...
getInspectionVen: function (data, cb) {
    var cnd = "",_this=this;
    if (typeof data.company != 'undefined') {
        cnd += " AND company='" + data.company + "' ";
    }
    ...
    if (typeof data.val != 'undefined') {
        cnd += " AND (label LIKE '%" + data.val + "%' ) ";
    }
    var sql = "SELECT id,label FROM inspection  where 1"+cnd+" ORDER BY id DESC LIMIT "+data.start+","+data.tot;
    this.selectCustomDb(sql, function (row) {
        sql = "SELECT count(id) tot FROM  inspection   where 1"+cnd;
    _this.selectCustomDb(sql, function (tot) {
        return cb([row,tot]);
    });
    });

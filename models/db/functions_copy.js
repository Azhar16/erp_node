...
var dp = exports.poolDatabase = {
    ...
    getproditemcodificationVen: function (data, cb) {
        ...
        if (typeof data.val != 'undefined') {
            cnd += " AND (pc.product_item_id LIKE '%" + data.val + "%' OR i.name LIKE '%" + data.val + "%' OR i.specification LIKE '%" + data.val + "%' OR i.unit LIKE '%" + data.val + "%') ";
        }
        var sql = "SELECT pc.id,pc.prod_specification_category,pc.prod_specification_item,pc.product_item_id,i.id as itemid,i.name,i.specification,i.unit FROM product_codification pc INNER JOIN item i ON pc.product_item_id = i.code  where 1"+cnd+" GROUP BY pc.product_item_id ORDER BY pc.id DESC LIMIT "+data.start+","+data.tot;
        console.log(sql);
        this.selectCustomDb(sql, function (row) {
            sql = "SELECT count(DISTINCT (pc.product_item_id)) tot FROM  product_codification pc INNER JOIN item i ON pc.product_item_id = i.code  where 1"+cnd;
    ...
    getOfferEdit:function(data,cb){
        ...
        this.selectCustomDb(sql, function (row) {
            row.forEach(async function (sa) {
                ...
                arr.item=await _this.asyncselectCustomDb("SELECT p.id,i.quantity,i.unit punit,i.rate,i.tax,i.discount,i.profit,i.final_amount,i.tax_amount,i.description,p.name,p.hsn_code,p.specification,p.tax_slabe,p.unit,p.unit_two,p.unit_three FROM `offer_item` i INNER JOIN item p ON p.id=i.item WHERE i.offer='" + data.id + "'");
       ...
       // console.log("SELECT id,DATE_FORMAT(offer_date,'%m/%d/%Y') offer_date,amendment_no,amendment_date,offer_status,customer,offer_no,offer_note,offer_validity,total_amount,offer_term_note FROM `offer` WHERE id='" + data.id + "' ");
        this.selectCustomDb(sql, function (row) {
            row.forEach(async function (sa) {
                ...
                arr.item=await _this.asyncselectCustomDb("SELECT p.id,i.quantity,i.offer as offerid,i.unit punit,i.rate,i.discount,i.profit,i.final_amount,i.tax as itax,i.tax_amount,i.description,p.name,p.hsn_code,p.specification,p.tax_slabe,p.unit,p.unit_two,p.unit_three,i.tax FROM `offer_item` i INNER JOIN item p ON p.id=i.item WHERE i.offer='" + data.id + "'");
                arr.newitem=await _this.asyncselectCustomDb("SELECT p.id,p.code,i.quantity,i.unit punit,i.rate,i.discount,i.profit,i.final_amount,i.tax_amount,i.tax,i.description,p.name,p.hsn_code,p.specification,p.tax_slabe,p.unit,p.unit_two,p.unit_three FROM `workorder_item` i INNER JOIN item p ON p.id=i.item WHERE i.offer='" + data.id + "' AND i.workorder='"+data.woid+"'");
    ...
    var sql = "SELECT o.id,o.offer_no,DATE_FORMAT(o.offer_date,'%d, %b %Y') offer_date,c.company_name,c.address,c.state,c.zip,c.city,c.gst,c.code,o.offer_term_note,t.slab,t.cgst,t.igst,t.sgst,o.total_amount,o.offer_note FROM `offer` o INNER JOIN customer c on c.id = o.customer INNER JOIN tax_tran t ON t.tran_id=o.id WHERE t.type='offer'AND o.id='"+data.id+"'";
    this.selectCustomDb(sql, function (row) {
        return cb(row);
      });
   },
   ...
   getBillOfferAdditional: function (data, cb) {
    ...
    getAssetEdit:function(data,cb){
        var arr={},_this=this;
        var sql = "SELECT id,asset_no,asset_type,manufacture,model,default_price,status,asset_name FROM asset WHERE id='" + data.id + "' ";
        this.selectCustomDb(sql, function (row) {
            row.forEach(async function (sa) {
                arr.sa=sa;
...
getAssetlocationVen: function (data, cb) {
    ...
    getAcureAssetVen: function (data, cb) {
        ...
        var sql = "SELECT ai.id,DATE_FORMAT(ai.date,'%d, %b %Y') asset_date,ai.asset_validity,c.name,c.company_name FROM asset_info ai INNER JOIN customer c ON c.id=ai.vendor   where 1"+cnd+" ORDER BY ai.id DESC LIMIT "+data.start+","+data.tot;
        console.log(sql);
        this.selectCustomDb(sql, function (row) {
            sql = "SELECT count(ai.id) tot FROM asset_info ai INNER JOIN customer c ON c.id=ai.vendor  where 1"+cnd;
    ...
    getAcureAssetEdit:function(data,cb){
        var arr={},_this=this;
        var sql = "SELECT id,DATE_FORMAT(date,'%m/%d/%Y') date,asset_validity,location,asset_note,vendor FROM `asset_info` WHERE id='" + data.id + "' ";
        this.selectCustomDb(sql, function (row) {
            row.forEach(async function (sa) {
                arr.sa=sa;
                arr.cus=await _this.asyncselectCustomDb("SELECT * FROM `customer`  WHERE id='" + sa.vendor + "' ");
                arr.item=await _this.asyncselectCustomDb("SELECT a.id,ai.quantity,ai.rate,ai.final_amount,ai.description,a.asset_name,a.asset_no FROM `asset_item` ai INNER JOIN asset a ON a.id=ai.asset WHERE ai.asset_info='" + data.id + "'");
                arr.loc=await _this.asyncselectCustomDb("SELECT * FROM `asset_location`  WHERE id='" + sa.location + "' ");
                arr.img=await _this.asyncselectCustomDb("SELECT * FROM `document`  WHERE ref_id='" + sa.id + "' AND doc_type='acure_asset' ");
                return cb(arr);
            });
        });
    },
    ...
    getofferMappingVen:function(data,cb){
        ...
        var sql = "SELECT o.id,i.id as itmid,i.code,cmd.size_id,i.description,oi.description as desp FROM offer o INNER JOIN offer_item oi ON o.id=oi.offer INNER JOIN item i ON oi.item=i.id INNER JOIN component_mapping_details cmd ON cmd.item=i.id WHERE 1"+cnd+" ORDER BY oi.id DESC LIMIT "+data.start+","+data.tot;
        //console.log(sql);
        this.selectCustomDb(sql, function (row) {
    ...
    newMappingdetailsByItemId:function(data,cb){
        var itm=[],_this=this;
        var sql = "SELECT im.id as imid,im.material,im.item,i.code,i.hsn_code FROM item_mapping im INNER JOIN item i ON i.id = im.material  WHERE im.item='"+data.itm+"' AND im.offer='"+data.ofr+"' ";
        this.selectCustomDb(sql, function (row) {
          row.forEach(async function (sa) {
              var citem=await _this.asyncselectCustomDb("SELECT cm.id,cm.item,cm.prod_quantity,ccn.id as ccnid,ccn.component_name,pf.id as pfid,pf.name as pfname ,psf.id as psfid,psf.name as psfname FROM component_mapping cm INNER JOIN component_category_name ccn ON cm.component = ccn.id INNER JOIN product_feature pf ON cm.component_feature = pf.id LEFT JOIN product_sub_feature psf ON psf.id = cm.component_sub_feature WHERE cm.item='"+sa.item+"'");
                itm.push({imid:sa.imid,material:sa.material,code:sa.code,hsn_code:sa.hsn_code,citem:citem[0].item,prod_quantity:citem[0].prod_quantity,ccnid:citem[0].ccnid,pfid:citem[0].pfid,pfname:citem[0].pfname,psfid:citem[0].psfid,psfname:citem[0].psfname});
                return cb(itm);
            });
      });
    },

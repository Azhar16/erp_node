var PENQUIRY = {
    ...
    initPEN:function(comstate,tax_slab,ship){
        ...
        jQuery('#expected_delivery_date').datepicker({
                autoclose: true,
                todayHighlight: true
            });
       ...
       /* $("#shipping").on("change", function() {
        ...
        $("#office_type").on("change", function() {
            $.get('/purchase/purchase_enquiry/autonum/' + $(this).val(), function (data) {
                var sd = JSON.parse(data);
                $("#enquiry_no").val(sd);
    ...
    submitSNewFrm: function () {
        ...
        $( ".data-row-wrap" ).each(function( i ) {
            ...
            if(id>0){
                ...
                console.log("h "+itm.length);
            }
        });
        console.log("f "+itm.length);
        //console.log("g "+itm.doc.length);
        $(".safildwrap").each(function (i) {
        ...
        var data = {vendor: $("#vendor_name").val(),
            ...
            enquiry_note: $("#enquiry_note").val(),
            enquiry_terms_note: $("#enquiry_terms_note").val(),
        ...
        $.ajax({
            ...
            complete: function (xhr) {
                ...
                if (res.code > 0) {
                    //if(drawingdoc != ''){  
                      PENQUIRY.submitdocNewFrm(res.code);
                     // }
                    $('#mainContent').html("<div class='loading'></div>");
      ...
      $('#frmSNew').parsley();
        ...
        $(".doc-dlt").on("click", function (e) {
            ...
            swal({
                ...
                $.get('/purchase/purchase_enquiry/docdelete/' + cid, function (data) {
                    $.get("/purchase/purchase_enquiry", function (data) {
                        $('#mainContent').html(data);
    ...
    submitdocNewFrm: function (id) {
        ...
        $.ajax({
            ...
            processData: false,
            contentType: false,
            url: '/purchase/purchase_enquiry/docsave',
            complete: function (xhr) {
                var res = JSON.parse(xhr.responseText);
                if (res.code > 0) {
                    $('#mainContent').html("<div class='loading'></div>");
